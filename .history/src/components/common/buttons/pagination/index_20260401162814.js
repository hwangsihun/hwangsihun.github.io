...
function createPaginationButton(element) {
    ...
    element.innerHTML = `
        <button
            type="button"
            class="group flex h-[50px] w-[50px] items-center justify-center rounded-full border bg-white text-[#191f28] transition-all duration-300 ease-in-out c_lightGrayLine hover:bg-black hover:text-white"
            aria-label="${label}"
        >
            <img
                src="${iconConfig.src}"
